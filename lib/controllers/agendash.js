...
// NOTE: Not always all failed jobs finish with "failedAt" same as "lastFinishedAt" date (a diff of some milliseconds). 
// So there is a need for a threshold to avoid false-positive completed jobs.
const FINISHED_AT_THRESHOLD_OFFSET = 50
...
module.exports = function (agenda, options) {
        ...
        },
        {
          $project: {
            ...
            completed: {
              $and: [
                "$lastFinishedAt",
                { $gt: [dateFieldMinusOffset("$lastFinishedAt"), "$failedAt"] },
              ],
            },
            failed: {
              $and: [
                "$lastFinishedAt",
                "$failedAt",
                { $lte: [dateFieldMinusOffset("$lastFinishedAt"), "$failedAt"] },
                { $gte: ["$lastFinishedAt", "$failedAt"]}
              ],
            },
            repeating: {
  ...
  // Create a threshold date to manage failed jobs
  const dateFieldMinusOffset = function(field, offset = FINISHED_AT_THRESHOLD_OFFSET) {
    return {"$add": [new Date(0), {"$subtract": [{"$toLong": field}, offset]}]}
  }
